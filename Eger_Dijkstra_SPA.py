...
# Only difference between LAZY APPROACH and EGER APPROACH is instead of inserting all the key value pair
# of node and distance it updates the path with the shortest path which is more effecient.
        ...
        if dist_of_current is not None or current_dist < dist_of_current:
            for edge in allEdges:
                ...
                if Dist[np.where(Nodes == edge[1])][0] is None:
                    ...
                    NodeQueue.update(edge[1], new_dist)
                    Dist[np.where(Nodes == edge[1])] = new_dist
